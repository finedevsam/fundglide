...
public class BulkPaymentServiceImpl implements BulkPaymentService {
    ...
    @Override
    public ResponseEntity<?> bulkPayment(BulkPaymentDto bulk) {
                ...
                for(Row row: sheet){
                    if(row.getRowNum() == 0){
                        ...
                        if (!accountRepository.existsByAccountNo(receiverAccountNo)){
                            return response.failResponse("%s is not a valid account number. Kindly fix and try again".formatted(receiverAccountNo), HttpStatus.BAD_REQUEST);
                        }
