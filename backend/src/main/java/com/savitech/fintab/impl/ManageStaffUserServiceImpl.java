...
public class ManageStaffUserServiceImpl implements ManageStaffUserService{
    ...
    @Override
    @SneakyThrows
    public ResponseEntity<?> createStaff(CreateStaffDto staffModel) {
        ...
        for(String i: staffModel.getPermission()){
            if(!permissionRepository.existsById(i)){
                return response.failResponse("`%s` is not a valid permission".formatted(i), HttpStatus.BAD_REQUEST);
            }
        }
    ...
    @Override
    public ResponseEntity<?> adminUpdateStaff(String Id, UpdateStaffDto updateStaffModel) {
        User user = authenticatedUser.auth();
    ...
    @Override
    public ResponseEntity<?> giveUserPermission(String Id, AddPermissionToStaffDto permissionToStaff) {
        User user = authenticatedUser.auth();
