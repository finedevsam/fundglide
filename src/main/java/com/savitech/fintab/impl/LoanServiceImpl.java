...
public class LoanServiceImpl implements LoanService{
    ...
    @Override
    public ResponseEntity<?> createLoanType(LoanTypeDto loanTypeModel) {
    ...
    @Override
    public ResponseEntity<?> updateLoanType(String Id, LoanTypeDto loanTypeModel) {
        User user = authenticatedUser.auth();
    ...
    @SneakyThrows
    @Override
    public ResponseEntity<?> applyForLoan(ApplyForLoanDto loan) {
        User user = authenticatedUser.auth();
    ...
    @Override
    public ResponseEntity<?> configureLoan(LoanConfigDto loanConfigDto) {
       User user = authenticatedUser.auth();
        if(!user.getIsAdmin()){
            return response.failResponse("You don't have permission to perform this opeation", HttpStatus.BAD_REQUEST);
        }
        ...
        List<String> loanConfList = Arrays.asList("disburse", "interest");

        if(Objects.equals(loanConfigDto.getType(), "") || Objects.equals(loanConfigDto.getType(), null)){
            return response.failResponse("type can not be empty", HttpStatus.BAD_REQUEST);
        }

        if(!loanConfList.contains(loanConfigDto.getType())){
            return response.failResponse(String.format("invalid configuration type. type can only take %s as value", loanConfList), HttpStatus.BAD_REQUEST);
        }

        if(Objects.equals(loanConfigDto.getSourceId(), "") || Objects.equals(loanConfigDto.getSourceId(), null)){
            return response.failResponse("Source id can't be null or empty", HttpStatus.BAD_REQUEST);
        }

        if(!internalAccountRepository.existsById(loanConfigDto.getSourceId())){
            return response.failResponse("Invalid internal account", HttpStatus.BAD_REQUEST);
        }

        if(loanConfigRepository.existsByType(loanConfigDto.getType())){
            return response.failResponse(String.format("Configuration already set for %s", loanConfigDto.getType().toUpperCase()), HttpStatus.BAD_REQUEST);
        }

        LoanConfig config = new LoanConfig();
        config.setName(loanConfigDto.getName());
        config.setSourceId(loanConfigDto.getSourceId());
        config.setType(loanConfigDto.getType());
        loanConfigRepository.save(config);
        return response.successResponse("Loan Configuration set", HttpStatus.BAD_REQUEST);
    }

    @Override
    public ResponseEntity<?> loanConfiguration() {
        User user = authenticatedUser.auth();
        if(!user.getIsAdmin()){
            return response.failResponse("You don't have permission to perform this opeation", HttpStatus.BAD_REQUEST);
        }
        ...
        return ResponseEntity.ok().body(loanConfigRepository.findAll());
    }
